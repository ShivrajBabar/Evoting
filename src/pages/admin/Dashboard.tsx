  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const adminConstituencyId = Number(user?.vidhansabha_id);



        const filteredVoters = votersData.filter(
          (v: Voter) => Number(v.vidhansabha_ward_id) === adminConstituencyId
        );
        ...
        const filteredElections = electionsData
          .filter(
            (e: Election) =>
              Number(e.vidhansabha) === adminConstituencyId &&
              (e.status === 'Scheduled' || e.status === 'Active')
          )
          .slice(0, 4);
        // Top 4 upcoming

        setVoters(filteredVoters.slice(0, 4));
      ...
      <div className="space-y-6">
        ...
        <Card className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white">
          <CardContent className="p-6">
            ...
            <p className="mt-2">
              You are managing {user?.constituency || 'your'} constituency
            </p>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Elections scheduled for your constituency
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                ...
                <div className="space-y-4">
                  {voters.map((voter) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-500">
                        Voter ID: {voter.voter_card_number}
                      </p>
                    </div>
                  ))}
                </div>
